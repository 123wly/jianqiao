...
class WechatRequest
{
     ...
     * @return array|string
     */
    public static function switchType(&$request, $callBack)
    {
        if($request['msgtype'] == "event"){
            if(method_exists($callBack,"__event_".strtolower($request['event']))){
                return call_user_func_array(array($callBack,"__event_".strtolower($request['event'])), array(&$request));
            }
        }else {
            if(method_exists($callBack,"__".$request['msgtype'])){
                return call_user_func_array(array($callBack,"__".$request['msgtype']), array(&$request));
            }
        }
//        $data = array();
...
//            case 'text':
//
//                if(method_exists($callBack,"__text")){
//                    return call_user_func_array(array($callBack,"__text"), array(&$request));
//                }
...
//            //语音
//            case 'voice':
//                if(method_exists($callBack,"__voice")){
//                    return call_user_func_array(array($callBack,"__voice"), array(&$request));
//                }
//                $data = self::voice($request);
//                break;
//            //视频
//            case 'video':
//                if(method_exists($callBack,"__video")){
//                    return call_user_func_array(array($callBack,"__video"), array(&$request));
//                }
//                $data = self::video($request);
//                break;
//            //位置
//            case 'location':
//                if(method_exists($callBack,"__location")){
//                    return call_user_func_array(array($callBack,"__location"), array(&$request));
//                }
//                $data = self::location($request);
//                break;
//            //链接
//            case 'link':
//                if(method_exists($callBack,"__link")){
//                    return call_user_func_array(array($callBack,"__link"), array(&$request));
//                }
...
//        return $data;
    ...
    {
        $content = '收到文本12消息';
        $tuwenList[] = array('title' => 'asdf', 'description' => '描述1', 'pic_url' => '图片URL1', 'url' => '点击跳转URL1');
