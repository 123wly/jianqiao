...
function show(index) {
    ...
    setTimeout(function () {
        ...
        adTxt.eq(index).css({ top: "0", opacity: "0" }).stop(false, true).animate({ top: "90px", opacity: "1" }, time);
        adImg.eq(index).css({ right: "-50px", opacity: "0" }).stop(false, true).animate({ right: "130px", opacity: "1" }, time);
    }, 200)
    curIndex = index;
}